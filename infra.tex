...
\begin{document}
	...
	\subsection{Basics}
		...
		\begin{itemize}
			...
			\item All non-static methods in interfaces are abstract and public\footnote{Hence the access modifiers do not need to be specified. When writing a class that implements an interface, the class cannot reduce the visibility of the methods from the interface, which means that those need to be public. Other additional methods can of course have any access level modifiers.}; however, public static methods in interfaces can be concrete. In abstract classes, abstract methods need to be declared as such (via the \texttt{abstract} keyword) and they can be public, protected or package private (but not private!). Abstract classes can also have non-abstract (concrete) methods, which can be public, protected, package private or private\footnote{Obviously, private non-abstract methods cannot be overriden, and private abstract methods do not exist.}.
	...
	\section{Access Level Modifiers}
	
	If a base class has a \texttt{protected} field, subclasses can access it (\textbf{also outside the package}). Any class within the same package can access a protected field.
