...
\begin{document}
	...
	\subsection{Merging}
	\begin{itemize}
		\item Assume that the following commit-history is given:
	\end{itemize}
	...
	\begin{tikzpicture}
		...
		\node at (5, 1.5) {G};
		\node at (6, 1.5) {H};
		\node at (7, 1.5) {branch <new_feature>};
	\end{tikzpicture}

	\begin{itemize} 
		\item When merging <main> and <new_feature>, the commits D and D'' will be either auto-merged or there will be a merge conflict. But of course, they will not be lost! 
		\item Now assume that a slightly different commit-history is given: 
	\end{itemize} 
	
	\begin{itemize}
		\item E -- F -- G -- H branch <new_feature>
		\item / 
		\item A -- B -- C -- D branch <main>
		\item \ 
		\item D' – D'' branch <cool_feature>
	\end{itemize}
	
	\begin{itemize}
		\item After merging <main> with <new_feature>, which happens via a fast-forward, we can delete it, as it is of no use to us anymore. The commit structure then looks like: 
	\end{itemize} 

	\begin{itemize} 
		\item A -- B -- C -- D -- E -- F -- G -- H branch <main>
		\item \ 
		\item To merge <new_feature> into master, do the following:
	\end{itemize}
