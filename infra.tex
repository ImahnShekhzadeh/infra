...
% --------------------------- APPENDIX -------------------------	
	...
	\begin{lstlisting}[style=mystylebash, label=alg:bashrc_contents, caption=Contents of .bashrc file, xleftmargin=\parindent]
		...
		# ---------------------------- AWS --------------------------
		# listing
		# NOTE: three arguments required
		# example usages:
		#    `lal my_bucket`
		#    `lal migrate https://kalousis.s3.unige.ch`
		#    `lal migrate https://kalousis.s3.unige.ch default`
		lal() {
			local bucket_name="$1"
			...
			# check if the second argument contains "https://"                                       
			if [[ "$2" == https://* ]]; then
				local endpoint_url="$2"
			
				# if there's a third argument, it's the profile
				if [ -n "$3" ]; then
					local profile="$3"
				fi
				...
				# if the second argument doesn't contain "https://", it's the profile
				local profile="$2"
			fi
			
			$(which aws) s3 ls s3://"$bucket_name" --endpoint-url "$endpoint_url" --profile "$profile"
		}
