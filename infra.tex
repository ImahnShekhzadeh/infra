...
\begin{document}
	...
	\begin{itemize}
		...
		\item Creating new directory including all parent directories,
		
		\begin{lstlisting}[style=mystylebash, label=alg:bash__dir_creation, xleftmargin=\parindent]
			mkdir -p <dir>
		\end{lstlisting}
		
		\texttt{-p} option is safe, since if directory is already existent, no error will be outputted.
		
		\item Searching for files with specific extension, e.g. \texttt{.ext}: 
		...
		Note that this can be nicely combined with \texttt{grep}.
		
		\item In Bash, \texttt{[[ ]]} over \texttt{[ ]} is preferred, since \texttt{[[ ]]} is safer and more capable within Bash scripts. Within \texttt{[ ]}, where word splitting and filename expansion do occur, it is good practice to double-quote variables. But it is safe to omit the double-quotes for e.g.~\$\# within \texttt{[[ ]]}.
	...
	\section{Opening Programs from CLI}
	...
	\begin{itemize}
		...
		\begin{enumerate}
			\item Boot into an older kernel version of Linux (in order to get there, do a "hard" reboot, and then go into \enquote{Advanced options for Ubuntu}, and choose an older kernel version). 
			...
			\item Now to fix the monitors (because dual-monitor setup didn't work), I had to open the program \enquote{Additional Drivers} and change the driver from the open-source version to an NVIDIA proprietary one. 
	...
	\begin{itemize}
		...
		when merging \texttt{main} and \texttt{new\_branch}, the commits D and D'' will be either auto-merged or there will be a merge conflict.
		...
		\item Merging \texttt{new\_feature} into \texttt{main},
	...
	\section{Merge Conflicts}
		...
		\begin{enumerate}
			...
			\item \texttt{git restore} does not overwrite \texttt{HEAD}, though. For that, a push would be necessary. It can sometimes also be useful to run dry merges to proactively check for conflicts during a merge; for this, run
			...
			to abort, and no changes will occur. Only for a fast-forward, do 
...
% --------------------------- APPENDIX -------------------------	
	...
	\begin{lstlisting}[style=mystylebash, label=alg:bashrc_contents, xleftmargin=\parindent]
		...
		# usage: 
		#     `lmv file1 file2 file3 [...] /path/to/target_dir`
		#     `lmv source_dir target_dir`
		lmv() {
				...
				# shift the arguments so that last argument (`target_dir`) is dropped
				unset args[-1]                                                                                                        
				...
				for path in "${args[*]}"; do
						if [[ -e "$path" ]]; then
								if [[ -d "$path" ]]; then
										# move each item in the directory individually
										for item in "$path"/*; do
