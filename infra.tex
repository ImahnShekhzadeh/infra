...
\title{Infra}
% \subtitle{Questions}
\author{Imahn Shekhzadeh \\ imahn.shekhzadeh@unige.ch\normalsize}
\date{\today}
...
\begin{document}
	...
	\newpage 
	\section{Linux (e.g.~bash commands)}
	\begin{itemize}
		\item If you want to download a file from an URL, use this command (better than \textit{wget}): 

		\begin{lstlisting}[style=mystylebash, label=alg:curl, caption=Curl, xleftmargin=\parindent]
			curl -Lo output.out https://url.com
		\end{lstlisting}
	
		\item Let's say your directory structure looks like this: 
		...
		\quad Now to rename all files, e.g.~to \textit{infra.*}, do this in bash: 
		\begin{lstlisting}[style=mystylebash, label=alg:linux_mv_files_for, caption=Renaming of files with for-loop, xleftmargin=\parindent]
		...
		\item Under Ubuntu, you can find \textit{all} available kernels like this: 
		...
		To find the currently \textit{active} kernel version, do 
		...
		\begin{lstlisting}[style=mystylebash, label=alg:ubuntu__line_appending, caption=Append line to file, xleftmargin=\parindent]
		...
		\item Checking whether a provided string (e.g.~via an argument) is empty or not (typically used within conditional statements): 
		...
		\begin{lstlisting}[style=mystylebash, label=alg:bash__args_shift, caption=Argument retrieval, xleftmargin=\parindent]
			test_sth(){
				shift
		...
		And of course there is nothing stopping us from doing this $N\geq 1$-times $\dots$ Pseudocode: 
		
		\begin{lstlisting}[style=mystylebash, label=alg:bash__args_shift_gen, caption=Argument retrieval (\textit{pseudocode}), xleftmargin=\parindent]
			test_sth(){
				shift
				...
				shift
		...
		\item To find out the size of a file or directory, use 
		
		\begin{lstlisting}[style=mystylebash, label=alg:dir_size, caption=Finding dir size, xleftmargin=\parindent]
		...
		\item To get the number of available CPU resources, do 
		...
		\item To open the settings from CLI, 
	...
	\begin{itemize}
		...
		\item Sometimes, when specifying the 
	\end{itemize} 
