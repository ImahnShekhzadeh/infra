...
\begin{document}
	...
	\subsection{Cloning}
	\begin{itemize}
		\item Cloning a repository via \texttt{sshfs} and adjusting the folder name, 

		\begin{lstlisting}[style=mystylebash, label=alg:cloning1, xleftmargin=\parindent]
			git clone ssh-url folder-name
			# git clone https-path folder-name
		\end{lstlisting}
	
		\item After moving into the \texttt{folder\_name} via cd, do 
	...
	\begin{itemize}
		...
		when merging \texttt{main} and \texttt{new\_branch}, the two branches will be either auto-merged or there will be a merge conflict.
		...
		\begin{tikzpicture}
			...
			% Position the branch labels
			\node[align=left] at (9, 1) {new\_branch};
			\node[align=left] at (9, -1) {cool\_branch};
			\node[align=left] at (9, 0) {main};
		\end{tikzpicture}

		\item After merging \texttt{main} with \texttt{new\_feature}, which happens via a fast-forward, you can delete it. New commit structure: 
	\end{itemize} 

	\begin{itemize} 
		\item A -- B -- C -- D -- E -- F -- G -- H \quad\quad main
	...
	\subsection{Merge Conflicts}
		...
		\texttt{git restore} does not overwrite \texttt{HEAD}, though. For that, a push would be necessary.  
