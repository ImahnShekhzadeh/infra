...
\begin{document}
	...
	To check whether the repository has been initialized, look out for the hidden file \texttt{.git}.
	
	\textbf{Note that this repository will not yet be visible remotely, e.g.~on GitLab or GitHub!}

	\section{\texttt{config}}
	
	Setting your username and e-mail:
	
	\begin{lstlisting}[style=mystylebash, label=alg:git_config, xleftmargin=\parindent]
		git config --global user.name "Blub blub"
		git config --global user.email "blub@blub.com"
	\end{lstlisting}
	
	The \texttt{--global} flag sets the configuration for all repositories on your machine. If you want to set the username and email for a specific repository only, omit the \texttt{--global} flag and run the commands inside that repository.
	
	To check that these settings were successful, run
	
	\begin{lstlisting}[style=mystylebash, label=alg:git_config_check, xleftmargin=\parindent]
	...
	\section{Branches}
		...
		\begin{lstlisting}[style=mystylebash, label=alg:git__branch_deletion_remote, xleftmargin=\parindent]
			git push origin --delete <branch_name>
		\end{lstlisting}
		
		To delete a local branch, first switch to another branch and then do
		\begin{lstlisting}[style=mystylebash, label=alg:git__branch_deletion_local, xleftmargin=\parindent]
			git branch -d <branch_name>  # if branch has already been merged into upstream branch
			git branch -D <branch_name>  # force deletion; irrespective of merge status (equivalent to `git branch --delete --force <branch_name>`)
		\end{lstlisting}
		
		If a branch is deleted remotely, it might still exist locally.
		
		When merging a branch into another one remotely, e.g.~via GitLab, and deleting it in the process, it can happen that \texttt{git branch -a} would indicate that the branch still exists. To correct this, 
		
		\begin{lstlisting}[style=mystylebash, label=alg:git_branch_del_prune, xleftmargin=\parindent]
			git fetch --prune
		\end{lstlisting}
	
		The branch might still exist locally, just run Alg.~\ref{alg:git__branch_deletion_local}.
		
		\subsection{Listing}
		
		List all remote and local branches:
		
		\begin{lstlisting}[style=mystylebash, label=alg:git__branch_list, xleftmargin=\parindent]
			git branch -a
		\end{lstlisting}
		...
		\subsection{Merging}
			...
			When merging \texttt{main} and \texttt{new\_branch}, the commits \texttt{D} and \texttt{D''} will either be auto-merged or there will be a merge conflict. In case the merge is successful or the merge conflicts have been resolved, there will be a new \textbf{merge commit} in the \texttt{main} branch. In case there was an auto-merge, git automatically creates the merge commit, otherwise the user needs to do this after resolving the merge conflicts.
			...
			merging \texttt{main} with \texttt{new\_feature} would result in a fast-forward (and no merge commit is created),
			...
			\begin{lstlisting}[style=mystylebash, label=alg:git_merge_abort, xleftmargin=\parindent]
				git merge --abort
			\end{lstlisting}
			
			When merging a branch into another one remotely, e.g.~via GitLab, and deleting it in the process, it can happen that \texttt{git branch -a} would indicate that the branch still exists. To correct this, cf.~Alg.~\ref{alg:git_branch_del_prune}.
	...
	\begin{itemize}
		...
		\item Note that if you also want to delete the file from the history, then you should use \texttt{git filter-repo}, cf.~Sec.~\ref{git:filter_repo}.
	\end{itemize}

	\section{\texttt{filter-repo}} \label{git:filter_repo}
		
		\subsection{Installation}

			To delete a file from the history, first install \href{https://github.com/newren/git-filter-repo/?tab=readme-ov-file}{\texttt{filter-repo}}:
			
			\begin{lstlisting}[style=mystylebash, label=alg:git_filter_repo__install, xleftmargin=\parindent]
		...
		\begin{lstlisting}[style=mystylebash, label=alg:git_filter_repo__use, xleftmargin=\parindent]
			git filter-repo --path PATH_TO_FILE --invert-paths --force
		\end{lstlisting}
		
		If \texttt{PATH\_TO\_FILE} still exists remotely, then add the remote repository:
		
		\begin{lstlisting}[style=mystylebash, label=alg:git_add_upstream, xleftmargin=\parindent]
			git remote add origin git@github.com:username/repository.git
		\end{lstlisting}
		
		And then verify this was successful via
		
		\begin{lstlisting}[style=mystylebash, label=alg:git_add_upstream_b, xleftmargin=\parindent]
			git remote -v
		\end{lstlisting}
		
		Now force-push.
	
	\section{\texttt{restore}}
	...
	To restore the content in the index, additionally use \texttt{--staged}:
	
	\begin{lstlisting}[style=mystylebash, label=alg:git_restore_staged, xleftmargin=\parindent]
		git restore --staged --source=<commit-hash> <file-path>
		# git restore --staged --source=HEAD README.md
	\end{lstlisting}

	\textbf{Only restore the index if you know what the implications are.} \\ \\
	To restore both the index and the working tree, add \texttt{--staged --worktree}:
	
	\begin{lstlisting}[style=mystylebash, label=alg:git_restore_staged_worktree, xleftmargin=\parindent]
		git restore --staged --worktree --source=<commit-hash> <file-path>
		# git restore --staged --worktree --source=HEAD README.md
	\end{lstlisting}

	\textbf{Only restore the worktree if you know what the implications are.} \\ \\
	If you want any of the changes to be permanent, type \texttt{git push} afterwards.
	...
	\begin{lstlisting}[style=mystylebash, label=alg:git_reset_soft, xleftmargin=\parindent]
		git reset HEAD~1
	\end{lstlisting}

	If you want to throw the last $n$ commits, then do
	
	\begin{lstlisting}[style=mystylebash, label=alg:git_reset_hard, xleftmargin=\parindent]
	...
	Note that neither Alg.~\ref{alg:git_reset_soft} nor Alg.~\ref{alg:git_reset_hard} will remove untracked files or directories. For this, do
	
	\begin{lstlisting}[style=mystylebash, label=alg:git_clean_forced, xleftmargin=\parindent]
		git clean -fd
	\end{lstlisting}

	\textbf{For this, please note that the untracked files will be irrevocably deleted!}
	...
	This command is also very useful to see the origin of branches. If you want more detailed information, e.g.~timestamps and author information, remove the \texttt{--oneline}:
	...
	\section{\texttt{ArrayList}}
		...
		\begin{itemize}
			\item In Java, \texttt{ArrayList} objects can be initialized directly:
			
			\begin{lstlisting}[style=mystylejava, label=alg:arraylist_init, xleftmargin=\parindent]
			...
			\item From Java version 7+, you can use the \textbf{diamond operator} to omit the generic type on the RHS of the declaration:
			
			\begin{lstlisting}[style=mystylejava, label=alg:arraylist_diam, xleftmargin=\parindent]
				import java.util.ArrayList;
			...
			\item To compare two \texttt{ArrayList} objects for equality, do \textbf{not} use the \texttt{==} operator; instead
			
			\begin{lstlisting}[style=mystylejava, label=alg:arraylist_comp, xleftmargin=\parindent]
				import java.util.ArrayList;
	...
	\subsection{Basics}
		...
		\begin{itemize}
			...
			\item Classes, abstract classes and interfaces can be either public or package-private.

			\item Abstract classes can have constructors; interfaces, on the other hand, cannot.
			
			\item When inheriting from a class, the subclass must implement the constructor if the super class has a non-default constructor and call the constructor from the super class.
			
			\item Abstract classes can contain public, protected, package private and private fields. In interfaces, fields are always \texttt{public static final}, i.e. there are no instance fields.
			
			\item All non-static methods in interfaces are implicitly abstract and public\footnote{Hence the access modifiers do not need to be specified. When writing a class that implements an interface, the class cannot reduce the visibility of the methods from the interface, which means that those need to be public. Other additional methods can of course have any access level modifiers.}; however, static methods in interfaces can be concrete. Also, static methods in interfaces can be either public (which they are implicitly, cf. \href{https://docs.oracle.com/javase/specs/jls/se8/html/jls-9.html#jls-9.4}{here}\footnote{\begin{quote}
			...
			\end{quote}}) or private, but not protected or package private (since package private would be specified by not adding any access level modifier, but in interfaces, methods are implicitly public).\footnote{It is generally the case that static methods cannot be \texttt{protected}, since they belong to the type where they are defined.}
			
			\item In abstract classes, abstract methods need to be declared as such (via the \texttt{abstract} keyword) and they can be public, protected or package private (but not private!). Abstract classes can also have non-abstract (concrete) methods, which can be public, protected, package private or private\footnote{Obviously, private non-abstract methods cannot be overriden, and private abstract methods do not exist.}.
			
			\item An abstract class can implement an interface, yet does not have to implement the abstract methods; this has to be done by subclasses inheriting from the abstract class.
			
			\item When implementing an interface's method or an abstract method from an abstract class, it is recommended to use the \texttt{@Override} tag for better readability and maintenance.
			
			\item When implementing a static method in a class, it can only refer to static fields.
			
			\item The \texttt{final} keyword can be used independently from \texttt{sealed} classes, cf. Sec. \ref{subsec:java_sealed}, and means that no class can inherit from a final class.
			
			\item In order to extend interfaces, it is possible to add \texttt{default} methods, which can be overriden in the classes implementing the interface.
			
			\begin{lstlisting}[style=mystylejava, label=alg:java_doc_tags_inheritance, xleftmargin=\parindent]
			...
			\item When inheriting from another class and overriding (not overloading) a method from the super class, you can use the \texttt{@Override} tag to make this clear. While this is not strictly necessary, it makes the code better readable and it is easier to maintain.
	...
	\begin{lstlisting}[style=mystylejava, label=alg:java_interfaces, xleftmargin=\parindent]
		public interface InvestmentCalculator {
			[...]
		}
		
		public class ExponentialInvestmentCalculator implements InvestmentCalculator{
			[...]
		}
		
		// In a main function:
		ExponentialInvestmentCalculator calculator = new ExponentialInvestmentCalculator();
		boolean instanceCheck = calculator instanceof InvestmentCalculator;  // `true`
	\end{lstlisting}

	With \textbf{polymorphism}, you can also make \texttt{calculator} an object, which has as type the interface \texttt{InvestmentCalculator}:
	
	\begin{lstlisting}[style=mystylejava, label=alg:java_interfaces_b, xleftmargin=\parindent]
		// In a main function:
		InvestmentCalculator calculator = new ExponentialInvestmentCalculator();
		boolean instanceCheck = calculator instanceof InvestmentCalculator;  // `true`
	\end{lstlisting}	
	
	\subsection{\texttt{sealed}}\label{subsec:java_sealed}
	
	It is possible to define \texttt{sealed} classes, \texttt{sealed} abstract classes and 
	\texttt{sealed} interfaces in Java17+. The keyword has the effect that it restricts which other 
	classes or interfaces may extend or implement them. For example:
	
	\begin{lstlisting}[style=mystylejava, label=alg:java_sealed, xleftmargin=\parindent]
		abstract sealed class Daimler extends Car permits EKlasse, CKlasse
		{
			void bite(){};
	...
	Here, the class \texttt{Daimler} inherits from the class \texttt{Car} and only allows the subclasses \texttt{EKlasse} and \texttt{CKlasse}, which will need to be defined.
	
	The subclasses \texttt{EKlasse} and \texttt{CKlasse} can now either be \texttt{final}, i.e. no other class can inherit from them, \texttt{sealed}, or \texttt{non-sealed}, which means that any subclass can inherit from the class (note that a sealed class cannot prevent its permitted subclasses from doing this). For example:
	
	\begin{lstlisting}[style=mystylejava, label=alg:java_sealed_subclass, xleftmargin=\parindent]
		final class EKlasse extends Daimler
		{ 
			@Override
			void bite()
			{
	...
	Also, according to the \href{https://docs.oracle.com/en/java/javase/17/language/sealed-classes-and-interfaces.html}{documentation}, any permitted subclasses must be accessible by the sealed class at compile time.
	
	\section{Commenting}

	For multi-line comments, use \texttt{/* comment...*/}, but if you want to comment a method, use \texttt{/** doc string...*/}.
	...
	\begin{lstlisting}[style=mystylebash, label=alg:java_html_doc_generation, xleftmargin=\parindent]
		javadoc -private -d doc `file1.java` `file2.java`  [...]
		# javadoc -private -d doc Account.java SavingsAccount.java
	\end{lstlisting}

	If you intend to publish the docs, then remove the \texttt{-private} option.

	\subsection{Tags}
	
	\begin{itemize}
		\item \texttt{@see}: Reference other functions; these references are clickable. Example:
		
		\begin{lstlisting}[style=mystylejava, label=alg:java_doc_tags_@see, xleftmargin=\parindent]
			public class Account {
				protected double savings;
			...
			public class SavingAccount extends Account {
				...
				/**
				* Saving account discourages money being withdrawn through a 1% relative penalty relative to the amount that is withdrawn.
				*
		...
		\begin{lstlisting}[style=mystylejava, label=alg:java_doc_tags_@throws, xleftmargin=\parindent]
			public class Account {
		...
		\item \texttt{@deprecated}:~Used to indicate that a method or class is deprecated.
		\item \texttt{@link}:~Used to create hyperlinks in comments; references methods, constructors, fields or other classes in the documentation. 
		
		\begin{lstlisting}[style=mystylejava, label=alg:java_doc_tags_@link, xleftmargin=\parindent]
			public class Account {
				protected double savings;
			...
			public class SavingAccount extends Account {
				...
				/**
				* Saving account discourages money being withdrawn through a 1% relative {@link withdrawalPenalty penalty} relative to the amount that is withdrawn.
				*
				* @param amount Amount to be withdrawn.
				* @return 
				* @see Account#withdraw(double)
		...
		Note that is not necessary to use the \texttt{@link} tag when using the \texttt{@see} tag, since \texttt{@see} automatically leads to a hyperlink.
