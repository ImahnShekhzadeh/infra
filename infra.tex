...
\begin{document}
	...
	\subsection{PyTorch}
		...
		\subsubsection{Leaf Tensors}
			...
			\begin{itemize}
				\item If the \textit{requires\_grad} of tensors is \textit{False}, then the created leaf tensor will be leaf by convention. If \textit{requires\_grad} is \textit{True}, then the tensor will be leaf if it was created directly by the user and is \textbf{not} the result of an operation.
				
				\item However, by definition, leaf tensors themselves do not have a \textbf{gradient function} because they are not the result of a differentiable operation applied to other tensors, i.e.~\textit{grad\_fn} on such tensors will return \textit{None}. The gradient function in neural network libraries like PyTorch or TensorFlow is associated with tensors that are outputs of differentiable operations.
		...
		\subsubsection{Autograd \& Backward}
			...
			\begin{itemize}
				\item The function \texttt{torch.autograd.grad()} computes the gradient. It is particularly useful if more direct control over the gradient computation is desired, in particular compared to \texttt{.backward()}.
